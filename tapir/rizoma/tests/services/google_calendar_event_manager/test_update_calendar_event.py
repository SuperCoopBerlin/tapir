...
class TestUpdateCalendarEvent(FeatureFlagTestMixin, SimpleTestCase):
    @patch.object(GoogleCalendarEventManager, "get_api_client")
    def test_updateCalendarEvent_featureFlagIsOff_doesNothing(
        ...
        GoogleCalendarEventManager.update_calendar_event(attendance)

        mock_get_api_client.assert_not_called()

    @patch.object(GoogleCalendarEventManager, "get_api_client")
    def test_updateCalendarEvent_eventDoesntExist_doesNothing(
        self, mock_get_api_client: Mock
    ):
        self.given_feature_flag_value(
    ...
    @patch.object(GoogleCalendarEventManager, "get_api_client")
    def test_updateCalendarEvent_eventDoesntExist_updateEventAndSetExternalId(
        self, mock_get_api_client: Mock
    ):
        self.given_feature_flag_value(
            FEATURE_FLAG_GOOGLE_CALENDAR_EVENTS_FOR_SHIFTS, True
        ...
        update_api_endpoint = Mock()
        events_client.update = update_api_endpoint

        GoogleCalendarEventManager.update_calendar_event(attendance)

        update_api_endpoint.assert_called_once_with(
            calendarId="primary",
            eventId="test_id",
            body={
        ...
        update_api_endpoint.return_value.execute.assert_called_once_with()

...
class ShareOwnerMembershipConfirmationFileView(
    ...
    def get(self, request, *args, **kwargs):
        ...
        filename = "Mitgliedschaftsbestätigung {}.pdf".format(
...
class ShareOwnerFilter(django_filters.FilterSet):
    ...
    def filter_by_join_date(queryset: ShareOwner.ShareOwnerQuerySet, name, value):
        ...
        if value.start or value.stop:
            ...
            earliest_dates = (
                ShareOwnership.objects.filter(share_owner=OuterRef("pk"))
                .order_by("start_date")
                .values("start_date")[:1]
            )

            # Annotate with first share date
            queryset = queryset.annotate(first_share_date=Subquery(earliest_dates))
...
class RequestShareView(LoginRequiredMixin, CurrentShareOwnerMixin, generic.FormView):
    ...
    def form_valid(self, form):
        ...
        filename = "Beteiligungserklärung {}.pdf".format(
